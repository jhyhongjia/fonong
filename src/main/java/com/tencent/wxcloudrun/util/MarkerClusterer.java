...
public class MarkerClusterer {
    public ClusterResponse cluster(ClusterRequest request) {
        // 参数解析
        //窗口的可视经纬度范围
        ...
        if (lngSpan <= 0 || latSpan <= 0 || viewWidth <= 0 || viewHeight <= 0) {
            return new ClusterResponse(Collections.emptyList(), visiblePoints);
        }
        ...
        for (Point point : visiblePoints) {
            int x = (int) Math.floor((point.getLng() - swLng) / gridLng);
            int y = (int) Math.floor((point.getLat() - swLat) / gridLat);
            String gridKey = x + "," + y;
        ...
        for (Map.Entry<String, List<Point>> entry : gridMap.entrySet()) {
            ...
            if (gridPoints.size() > 1) { // 聚合点
                clusters.add(new Cluster(centerLng, centerLat, gridPoints.size()));
            } else { // 单点
                singlePoints.addAll(gridPoints);
            }
        }

        return new ClusterResponse(clusters, singlePoints);
    }
